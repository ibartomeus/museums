...
One of the main barriers to identifying long-term pollinator population trends is that pollinators are incredibly taxonomically diverse and include bees, flies, butterflies, beetles, birds, bats and lizards [@Ollerton2017]. Additionally, many pollinators are highly mobile, short-lived and small in size, which makes monitoring their populations difficult. Bees are generally regarded as the most important pollinator group due to their abundance, pollination efficiency and widespread distribution [@Winfree2011]. However, bees are diverse, with more than 20,000 species currently described worldwide [@Ascher2018], and often require expert taxonomists for identification. Furthermore, the uneven global distribution of research studies has resulted in geographical biases in bee decline research [@Martin2012], as well as taxonomic biases toward species that are easier to identify, such as bumblebees [@Archer2014; @DePalma2016].
...
```{r, include=FALSE}
#update joinCountryData2Map to avoid printing results
...
library(rworldmap)
map <- read.csv("../data/bee_richness.csv", sep = ";")
...
#map0 <- subset(map0, res > -2) #more contrasted plotting 
map2 <- joinCountryData2Map(map0, joinCode="NAME", nameJoinColumn = "country",
                            nameCountryColumn = "distribution", verbose = FALSE)
...
do.call(addMapLegend, c(map3, legendLabels="all", legendWidth=0.5, labelFontSize=0.8, legendMar = 4))
...
colorss <- c(heat.colors(9))
map.bee.ave <- mapCountryData(map.bee, nameColumnToPlot = "average", colourPalette= colorss,
                                  mapTitle = "Average bee richness change", catMethod = "pretty", addLegend = F)
do.call(addMapLegend, c(map.bee.ave, legendLabels="all", legendWidth=0.5, labelFontSize=0.8, legendMar = 4))
mtext("b)", side = 3, adj = 0.05, 
    line = -1.3)

#syrphid richness map
map_s <- subset(map0, is.na(syrphid_complete))
#scatter.smooth(log(map_s$syrphid) ~ log(map_s$size+1))
m <- lm(log(map_s$syrphid) ~ log(map_s$size+1), na.action = na.exclude) 
map_s$res <- residuals(m)
#subset(map0, res < -2)
map2 <- joinCountryData2Map(map_s, joinCode="NAME", nameJoinColumn = "country",
                            nameCountryColumn = "distribution", verbose = FALSE)
#need to use ISO's to make sure we don't carry over NA's due to country misspellings
#have plotted countries with missing values as grey (shows we have richness for pretty much everywhere)
map3 <- mapCountryData(map2, nameColumnToPlot = "res", missingCountryCol = "white",
               mapTitle = "Syrphid species richness", numCats = 25, catMethod = "pretty", addLegend = FALSE, mapRegion = "Europe")
#add a modified legend using the same initial parameters as mapCountryData
do.call(addMapLegend, c(map3, legendLabels="all", legendWidth=0.5, labelFontSize=0.8, legendMar = 4))
...
#average richness change
colorss <- c(heat.colors(5), c("#5e977c", "#3d88a5", "#1c79cf"))
map.syrphid.ave <- mapCountryData(map.syrphid, nameColumnToPlot = "average", colourPalette= colorss,
                       mapTitle = "Average syrphid richness change", catMethod = "pretty", addLegend = F, mapRegion = "Europe")
do.call(addMapLegend, c(map.syrphid.ave, legendLabels="all", legendWidth=0.5, labelFontSize=0.8, legendMar = 4))
mtext("d)", side = 3, adj = 0.05, 
    line = -1.3)

#butterfly richness
#scatter.smooth(log(map0$buterflies) ~ log(map0$size+1))
m <- lm(log(map0$syrphid) ~ log(map0$size+1), na.action = na.exclude) 
map0$res <- residuals(m)
#subset(map0, res < -2)
map2 <- joinCountryData2Map(map0, joinCode="NAME", nameJoinColumn = "country",
                            nameCountryColumn = "distribution", verbose = FALSE)
...
#have plotted countries with missing values as grey (shows we have richness for pretty much everywhere)
map3 <- mapCountryData(map2, nameColumnToPlot = "res", missingCountryCol = "white",
               mapTitle = "Butterfly species richness", numCats = 25, catMethod = "pretty", addLegend = FALSE, mapRegion = "Europe")
#add a modified legend using the same initial parameters as mapCountryData
do.call(addMapLegend, c(map3, legendLabels="all", legendWidth=0.5, labelFontSize=0.8, legendMar = 4))
...
#average richness change
colorss <- c(heat.colors(6))
map.butterfly.ave <- mapCountryData(map.butterfly, nameColumnToPlot = "average", colourPalette=colorss,
                                    mapTitle = "Average butterfly richness change", catMethod = "pretty", addLegend = F, mapRegion = "Europe")
do.call(addMapLegend, c(map.butterfly.ave, legendLabels="all", legendWidth=0.5, labelFontSize=0.8, legendMar = 4))
mtext("f)", side = 3, adj = 0.05, 
    line = -1.3)
par(mfrow = c(1,1))
```

**Figure 1:** Global map showing a), c) and e) bee, syrphid and butterfly species richness per area (Data from http://www.discoverlife.org, IUCN and http://www.syrphidae.com/). ALl richness levels are calculated as the residuals of the log-log regression between bee species richness per country and country size. This correction accounts for the species-area relationship. Warmer colours indicate higher bee diversity. Note that some African countries may have incomplete listed faunas and that Alaska is included with US values. Countries with available historical changes in (b) bee, (d) syrphid and (f) butterfly richness within the last 100 years. Warmer colours indicate steeper average declines. Countries without data are coloured in white. 
...
#plot all
par(mfrow = c(2,2),  mar = c(2.1, 3.1, 3.1, 2.1))
plot(log(lhf$post1980+1), log(lhf$pre1980+1), xlab = "post1980", ylab = "pre1980", col = col, pch = pchs, las = 1)
temp <- subset(lhf, country %in% c("ES", "NZ"))
text(x = temp$log_post1980, y = temp$log_pre1980+2 , labels = temp$country, col = "red")
abline(h = 7.5, v = 7.5)
...
map.gbif <- joinCountryData2Map(data2, joinCode="ISO2", nameJoinColumn = "countries",
                            nameCountryColumn = "countries", verbose = FALSE)
...
#dirty trick
est.all$taxa <- ifelse(est.all$taxa == "Fly", "b)     Fly", 
                                              "a)     Bee")
...
p <- p + geom_vline(data=filter(est.all, taxa=="a)     Bee"), aes(xintercept=5.5), colour="grey")
p <- p + geom_vline(data=filter(est.all, taxa=="b)     Fly"), aes(xintercept=7.5), colour="grey")
p <- p + geom_text(data=data.frame(Var1=3,estimate_frq=0.145,taxa="a)     Bee",lab='Exotic'), aes(label=lab), parse=TRUE,colour="grey70", size=3)
p <- p + geom_text(data=data.frame(Var1=4,estimate_frq=0.145,taxa="b)     Fly",lab='Exotic'), aes(label=lab), parse=TRUE,colour="grey70", size=3)
p <- p + geom_text(data=data.frame(Var1=16.5,estimate_frq=0.145,taxa="a)     Bee",lab='Native'), aes(label=lab), parse=TRUE,colour="grey70", size=3)
p <- p + geom_text(data=data.frame(Var1=11,estimate_frq=0.145,taxa="b)     Fly",lab='Native'), aes(label=lab), parse=TRUE,colour="grey70", size=3)
