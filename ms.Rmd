...
library(rworldmap)
map <- data.frame(iso2 = c("ES", "NZ"), richness = c(88, 75))
#FAKE data, we need to compile a bbdd with the data at hand.
map2 <- joinCountryData2Map(map, joinCode="ISO2", nameJoinColumn = "iso2", nameCountryColumn = "distribution")
mapCountryData(mapToPlot = map2, numCats = 2, nameColumnToPlot = "richness"
               , mapTitle = "Bee richness decline", addLegend = FALSE)
...
```{r low hanging fruits, echo=FALSE, message=FALSE, warning=FALSE, paged.print=FALSE}
library(rgbif)
...
country_codes <- c("ES", "NZ", "PT", "GE") #need to fetch them all EASY using rgbif
...
#lhf
plot(rowSums(lhf[,2:8]), rowSums(lhf[,9:15]), xlab = "post1980", ylab = "pre1980", col = as.factor(lhf$country))
abline(h = 3000, v = 5000)
#plot needs legend by country, and to be ggplotted
