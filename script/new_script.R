...
no.ind.fly <- as.data.frame(sort(table(date.record.fly$Gen_sp))) #14 species with more than 30 datapoints
...
#plot bee records
p <- ggplot(date.record.bee, aes(year, Freq, colour=Gen_sp))
p <- p + xlab("Year") + ylab("Number of records")
p <- p + theme(text = element_text(size=18))
p <- p + geom_point(size = 1)
p <- p + facet_wrap(~Gen_sp, scales="free")
p <- p + theme(panel.grid.minor = element_blank(),
...
p <- ggplot(xx2_join, aes(variable, estimate))
p <- p + xlab(NULL) + ylab("Number of fly species")
p <- p + theme(text = element_text(size=18))
p <- p + geom_point(size = 3)
p <- p + geom_abline(intercept = 19.8906, slope = -0.002058,linetype="dashed")#intercpt seems too high??
